...
@section('breadcrumb')
    ...
    @stop
@section('links')
    <a href="{!! route('category.create') !!}" class="btn pull-right hidden-sm-down btn-success"> <i class="mdi mdi-plus-circle"></i> Thêm danh mục</a>
@stop
@section('content')
    <div class="row">
        <div class="col-md-12">
                        ...
                        <table id="example23" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                           ...
                           @foreach($listCate as $key=>$item)
                            <tr  @if($key%2==0)
                                ...
                                ?></td>
                                <td>{{
                                    ($item->category_type=="post")?
                                    "Bài viết": "Sản phẩm"
                                }}</td>
                                <td>{!! $item->created_at !!}</td>
                                <td style="text-align: center">
                                    <a href="{{route('category.edit',$item->id)}}"><i class="mdi mdi-lead-pencil"></i></a>
            ...
            $(document).on('click','.delete',function () {
                if(confirm('Bạn có chắc muốn xóa?')){
                    ...
                    .done(function(data){
                        ...
                        swal("Xóa thành công", "Danh mục đã được xóa :)", "success");
                    })
                    .fail(function() {
                        swal("Đã xảy ra lỗi", "Danh mục chưa được xóa :)", "error");
                    });
                }
            });
        });
